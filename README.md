...
The current Agent release is `0.6.22-beta` on the **Breeze** channel.
...
- Accepts a bounded, server-managed Internet Relay polling policy and keeps
  critical Agent presence fixed at 30/90 seconds. The shared relay exposes a
  privacy-safe administrator usage report for cost tuning without PBX content.
...
the Agent recording endpoint. Grandstream can add CDR, voicemail, and recording
...
```bash
curl -fLO https://github.com/techmaniac89/pbxsense-agent/releases/latest/download/PBXSenseAgent-0.6.22-beta-linux-source-installer.tar.gz
curl -fLO https://github.com/techmaniac89/pbxsense-agent/releases/latest/download/SHA256SUMS.txt
sha256sum --check --ignore-missing SHA256SUMS.txt
tar -xzf PBXSenseAgent-0.6.22-beta-linux-source-installer.tar.gz
cd PBXSenseAgent-0.6.22-beta-linux-source-installer
```
...
- `PBXSENSE_INTERNET_RELAY_POLL_SECONDS`: changed-snapshot check cadence;
  ...
  five minutes.
- `PBXSENSE_SNAPSHOT_POLL_SECONDS`: PBX refresh cadence;
  defaults to `1` second and is clamped to at least `0.5` seconds.

Home and Signal payloads are cached until the next PBX snapshot. Relay
evaluation runs every five seconds, independently from the one-second PBX poll,
matching the proven `0.6.7-beta` runtime cadence.
...
- Creates a Python virtual environment and installs the hashed
...
those FreeSWITCH modules/files are available. Completed CDR is not exposed by
ESL, so `mod_json_cdr` output must be directly readable or mounted read-only
...
For Asterisk or Grandstream, declare exact trunk endpoint/profile names when
possible. This avoids relying on provider-name heuristics:

```text
PBXSENSE_TRUNK_ENDPOINTS=primary-carrier,backup-carrier
```

Unknown evidence is displayed as uncertain and does not start a new outage.
Explicit unavailable/registration failures must remain present for the trunk
confirmation interval before PBXSense emits a Health Signal.
...
The compose file uses the fixed Compose project name `pbxsense-agent` and
defines two named PBXSense volumes:
